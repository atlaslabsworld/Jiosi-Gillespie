...
\end{equation}
with rate-based discount $\beta\ge 0$. The objective is $\EX_{\tau\vert\pi_{\theta}}[F(\tau)]$; value functions similarly use state-action marginals such as $\EX_{s,a\vert\pi_{\theta}}[f(s,a)]$ without time discretization. This notation aligns with classical CTMDP treatments such as~\cite{guo2009ctmdp,  youtube_LMU_Frey_2025_Gillespie}.
...
Expert demonstrations are timestamped jump trajectories $\mathcal{D} = \{\tau^{(n)}\}_{n=1}^{N}$ with $\tau^{(n)} = \{(\tau_t^{(n)},\sigma_t^{(n)},s_t^{(n)},a_t^{(n)})\}$. The joint density for one step under parameters $\theta$ is $\lambda_{\sigma_t}(s_t,a_t)\allowbreak \exp(-\Lambda(s_t,a_t)\tau_t)\allowbreak \pi_{\theta}(a_t\vert s_t)$. The trajectory log-likelihood is
\begin{equation}
    \begin{aligned}
        \log p(\tau\vert\theta) = \sum_{t=0}^{T-1} \Bigl[
            &\log \lambda_{\sigma_t}(s_t,a_t)
            - \Lambda(s_t,a_t)\tau_t \\
